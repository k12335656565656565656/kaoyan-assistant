...
// 每个平台返回 { score: 0-100, source: 'real'|'search-fallback'|'failed', raw: {...} }
...
function parseCount(text) {
	return Number.parseInt(String(text || "").replace(/[,\s，]/g, ""), 10) || 0
}

function looksBlocked(engine, html, url = "") {
...
function extractSearchCount(html, site = "") {
	const patterns = [
		/百度为您找到相关结果约?\s*([\d,，]+)\s*个/,
		/找到相关结果约?\s*([\d,，]+)\s*个/,
		/为您找到相关结果约?\s*([\d,，]+)\s*个/,
	...
	for (const pattern of patterns) {
		...
		if (count > 0) return { count, source: "declared", resultBlocks: 0 }
	}

	const resultBlocks = [
		...(html.match(/class="[^"]*(?:result|c-container|result-op|vrwrap)[^"]*"/g) || []),
		...(html.match(/class=["'][^"']*b_algo[^"']*["']/g) || []),
		...(html.match(/data-click=["'][^"']*["']/g) || []),
	].length
	if (resultBlocks > 0) {
		return { count: resultBlocks * 1000, source: "result-blocks", resultBlocks }
	}

	if (site) {
		const domain = site.replace(/^www\./, "").replace(/\./g, "\\.")
		const domainHits = (html.match(new RegExp(domain, "gi")) || []).length
		if (domainHits >= 2) {
			return { count: domainHits * 800, source: "domain-hits", resultBlocks: domainHits }
		}
	}
	return { count: 0, source: "none", resultBlocks: 0 }
}

function scoreFromCount(count, scale = 12, floor = 0) {
	if (!count || count <= 0) return 0
	return Math.min(100, Math.round(Math.log10(Math.max(1, count)) * scale + floor))
}

async function fetchSearchFallback(platform, keyword, site, directError = "", options = {}) {
	if (options.signal?.aborted) {
		return failedScore(platform, `${directError || "direct failed"}; media timeout`)
	}
	const query = site ? `site:${site} ${keyword}` : keyword
	const engines = [
		{
			name: "baidu",
		...
		{
			name: "bing",
			url: `https://www.bing.com/search?q=${encodeURIComponent(query)}&setmkt=zh-CN`,
	...
	for (const engine of engines) {
		try {
			if (options.signal?.aborted) throw new Error("media timeout")
			await randDelay()
			const res = await fetch(engine.url, {
				headers: { ...DEFAULT_HEADERS, "Accept": "text/html", "Referer": engine.referer },
				redirect: "follow",
				signal: options.signal,
			})
			if (!res.ok) throw new Error(`${engine.name} HTTP ${res.status}`)
			const html = await res.text()
			if (looksBlocked(engine.name, html, res.url)) throw new Error(`${engine.name} blocked`)
			const parsed = extractSearchCount(html, site)
			if (parsed.count <= 0) throw new Error(`${engine.name} no search count`)
			return {
				score: scoreFromCount(parsed.count, options.scale || 12, options.floor || 0),
	...
	return failedScore(platform, `${directError || "direct failed"}; fallback failed: ${errors.join(" | ")}`)
}

// ===== B站 =====
async function fetchBilibili(keyword, options = {}) {
	try {
		await randDelay()
...
// ===== 知乎 =====
async function fetchZhihu(keyword, options = {}) {
	try {
		await randDelay()
		...
		return fetchSearchFallback("zhihu", keyword, "zhihu.com", e.message, { scale: 12, signal: options.signal })
	}
}

// ===== 百度 =====
async function fetchBaidu(keyword, options = {}) {
	try {
		await randDelay()
		const url = `https://www.baidu.com/s?wd=${encodeURIComponent(keyword)}`
		const res = await fetch(url, { headers: { ...DEFAULT_HEADERS, "Accept": "text/html" }, signal: options.signal })
		if (!res.ok) throw new Error(`HTTP ${res.status}`)
		const html = await res.text()

		const parsed = extractSearchCount(html)
		const count = parsed.count
		if (count === 0) throw new Error("no search count")

		// 评分：log10(count) * 12，封顶 100
		const score = scoreFromCount(count, 12)

		return {
			score,
			source: "real",
			raw: { resultCount: count, countSource: parsed.source, resultBlocks: parsed.resultBlocks },
		}
	} catch (e) {
		return fetchSearchFallback("baidu", keyword, "", e.message, { scale: 12, signal: options.signal })
	}
}

// ===== 抖音 =====
async function fetchDouyin(keyword, options = {}) {
	try {
		await randDelay()
		...
		return fetchSearchFallback("douyin", keyword, "douyin.com", e.message, { scale: 11, signal: options.signal })
	}
}

// ===== 贴吧 =====
async function fetchTieba(keyword, options = {}) {
	try {
		await randDelay()
		...
		return fetchSearchFallback("tieba", keyword, "tieba.baidu.com", e.message, { scale: 11, floor: 5, signal: options.signal })
	}
}

// ===== 微信 =====
async function fetchWechat(keyword, options = {}) {
	try {
		await randDelay()
		...
		return {
			score: Math.min(100, score),
			source: "real",
			raw: { sogouCount: count, countSource: parsed.source, resultBlocks: parsed.resultBlocks },
		}
	} catch (e) {
...
// ===== 小红书 =====
async function fetchXiaohongshu(keyword, options = {}) {
	try {
		await randDelay()
		...
		return fetchSearchFallback("xiaohongshu", keyword, "xiaohongshu.com", e.message, { scale: 11, signal: options.signal })
	}
}

// ===== QQ群 =====
async function fetchQQqun(keyword, options = {}) {
	try {
		await randDelay()
...
// ===== 主入口 =====
export async function fetchMediaHeat(school, major, options = {}) {
	...
	const results = await Promise.allSettled([
		fetchBilibili(keyword, fetchOptions),
		fetchZhihu(keyword, fetchOptions),
		fetchBaidu(keyword, fetchOptions),
		fetchDouyin(keyword, fetchOptions),
		fetchTieba(keyword, fetchOptions),
		fetchWechat(keyword, fetchOptions),
		fetchXiaohongshu(keyword, fetchOptions),
		fetchQQqun(keyword, fetchOptions),
	])
