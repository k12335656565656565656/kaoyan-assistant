...
from utils.database import MEMORY_DB, get_single_value, query_db
...
for fp in FONT_PATHS:
    ...
    if os.path.exists(fp):
        ...
        CN_FONT = 'ChineseFont'
        print(f"Using Chinese font: {fp}")
        break

if CN_FONT is None:
    print("No Chinese font found; report text may not render correctly")
    CN_FONT = 'Helvetica'
...
def generate_chart_image(fig, filename):
    ...
    handle, img_path = tempfile.mkstemp(prefix=f"kaoyan_{filename}_", suffix=".png")
    os.close(handle)
...
def generate_pdf_report(start_date, end_date, report_type):
    ...
    if not df_gap.empty:
        ...
        for i, row in enumerate(df_gap.itertuples(index=False), 1):
            row = (html.escape(str(row[0])), row[1])
