...
_LEGACY_LOCAL_MATERIAL_SOURCE_PROFILES = (
    LocalMaterialSourceProfile(
        key="exam_408",
        subject_label="408综合",
        title="本地 408 资料",
...
def _add_legacy_profiles(
    profiles: list[LocalMaterialSourceProfile],
) -> list[LocalMaterialSourceProfile]:
    """Keep old local-folder callers working after upstream catalog migration."""

    known_keys = {profile.key for profile in profiles}
    return profiles + [
        profile
        for profile in _LEGACY_LOCAL_MATERIAL_SOURCE_PROFILES
        if profile.key not in known_keys
    ]
...
LOCAL_MATERIAL_SOURCE_PROFILES: tuple[LocalMaterialSourceProfile, ...] = tuple(
    _add_legacy_profiles(_build_local_material_source_profiles(list_default_subject_profiles()))
)
...
def list_local_material_source_profiles(
    ...
    return _add_legacy_profiles(
        _build_local_material_source_profiles(
